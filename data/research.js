...
export default {
  ...
  intro: [
    ...
    'So the research began by testing access rather than by reading documentation. 101 endpoints were probed from a clean client, no account, no key, no subscription, no cookie. <strong>69 returned HTTP 200.</strong> Everything downstream follows from that result.'
  ],
  ...
  headline: [
    ...
    ['45.4%', 'carry an organisation identifier, the gap']
  ],
  ...
  findings: [
    {
      ...
      body: [
        ...
        'That releases the entire free portfolio, including OS Open UPRN and, critically, <strong>OS Open Linked Identifiers</strong>, which is exactly the property-to-street-to-topographic crosswalk the place spine needs.'
      ],
      note: 'The earlier working assumption, that the place spine required a licensed addressing product, was wrong for the identifier layer. The licensing line still holds for addresses, which is why UK GroundTruth emits identifiers and never address strings.',
      table: {
    ...
    },
    {
      ...
      body: [
        'The energy performance register returns <span class="mono">HTTP 200</span> to an anonymous client, and serves a sign-in page. A system built on documentation alone would have shipped with a dependency that silently returns nothing.',
  ...
  baseline: {
    ...
    lede: 'Computed from the full school register on 17 August 2026, 52,486 establishments, of which 27,167 are open.',
    rows: [
  ...
  cannot: {
    ...
    items: [
      ['Bid prices', 'Never published in UK procurement data. This kills the entire family of price-based collusion tests, so Sentinel relies on structural signals instead, shared directors, shared owners, framework concentration.'],
    ...
    close: 'The ceiling is set by what government chooses to publish, not by what can be computed. UK GroundTruth&rsquo;s proposition is that the published material already supports far more than is currently extracted from it, because the two joins that would unlock it have never been built.'
  },
  ...
  rules: {
    ...
    items: [
      ...
      ['Stay reproducible', 'Same inputs, same commit, same outputs, because a government buyer will eventually be challenged on a number by a named company.']
    ]
  }
};
